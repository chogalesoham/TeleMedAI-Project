...
const navigation: NavItem[] = [
    ...
    { name: 'Appointment Requests', path: '/doctor-dashboard/appointment-requests', icon: ClipboardList },
    { name: 'Confirmed Appointments', path: '/doctor-dashboard/confirmed-appointments', icon: CheckCircle },
    { name: 'Appointment History', path: '/doctor-dashboard/appointment-history', icon: History },
    { name: 'Live Consultation', path: '/doctor-dashboard/live-consultation', icon: Video },
    { name: 'Patient Queue', path: '/doctor-dashboard/patients', icon: Users },
    { name: 'Reports', path: '/doctor-dashboard/reports', icon: FileText },
    { name: 'Analytics', path: '/doctor-dashboard/analytics', icon: BarChart3 },
    { name: 'Settings', path: '/doctor-dashboard/settings', icon: Settings },
];
            ...
            <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm">
                <div className="flex items-center justify-between h-14 sm:h-16 px-3 sm:px-4 lg:px-6">
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <Link to="/doctor-dashboard" className="flex items-center gap-1.5 sm:gap-2">
                            ...
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="flex items-center gap-1.5 sm:gap-2 px-1.5 sm:px-2 h-9 sm:h-10">
                                    <Avatar className="h-7 w-7 sm:h-8 sm:w-8">
                                        <AvatarImage src={userData?.avatar} alt={userData?.name || 'Doctor'} />
                            ...
                            <DropdownMenuContent align="end" className="w-56">
                                <DropdownMenuLabel>
                                    <div className="flex flex-col space-y-1">
                                        <p className="text-sm font-medium">{userData?.name || 'Doctor'}</p>
                                        <p className="text-xs text-gray-500">{userData?.email || 'No email'}</p>
                                    </div>
                                </DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                <DropdownMenuItem asChild>
                                    <Link to="/doctor-dashboard/profile" className="flex items-center gap-2 cursor-pointer">
                                        <User className="h-4 w-4" />
                                        <span>Profile</span>
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem asChild>
                                    <Link to="/doctor-dashboard/settings" className="flex items-center gap-2 cursor-pointer">
                                        <Settings className="h-4 w-4" />
                                        <span>Settings</span>
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem asChild>
                                    <Link to="/doctor-dashboard/support" className="flex items-center gap-2 cursor-pointer">
                    ...
                    {!sidebarCollapsed && (
                        <div className="flex items-center gap-3">
                            <div className="flex flex-col">
                                <span className="text-sm font-bold text-gray-900">Doctor Portal</span>
                                <span className="text-xs text-gray-500">Medical Dashboard</span>
                            </div>
                        </div>
                    )}
